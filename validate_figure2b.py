...
MODIFIED FOR 8 ENCODER AND 8 DECODER LAYERS

Figure 1 claims:
  (a) A custom transformer (8 encoder layer, 8 decoder layer, 8 attention
...
This script reproduces the experiment on your own GPU:
  - Builds a transformer with 8 encoder and 8 decoder layers, 8 attention
    heads, d_model=512.
...
# --------------------------------------------------------------------------- #
# Custom transformer with 8 encoder and 8 decoder layers:
#   8 encoder layers, 8 decoder layers, 8 attention heads,
...
def plot_transformer_trace(ax, samples, phases, t_start):
    ...
    ax.set_title("(a) Transformer Model (8 Enc / 8 Dec Layers)")
...
def main():
    ...
    parser.add_argument("--output", type=str, default="figure1_8layers_validation.png")
    ...
    print("[1/2] Running custom Transformer "
          f"(8 enc / 8 dec layers, 8 heads, d_model=512, seq_len={args.seq_len}, "
    ...
    fig.suptitle("Power Trace: 8-Layer Transformer vs CNN (Modified Figure 1)", fontsize=14, fontweight='bold')
    plt.tight_layout()
    ...
    # Summary report
    print("\n" + "="*70)
    print("SUMMARY REPORT: 8-Layer Transformer Inference Power Analysis")
    print("="*70)
