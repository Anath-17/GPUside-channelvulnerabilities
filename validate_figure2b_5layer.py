...
MODIFIED FOR 5 ENCODER AND 5 DECODER LAYERS

Figure 1 claims:
  (a) A custom transformer (5 encoder layer, 5 decoder layer, 8 attention
...
This script reproduces the experiment on your own GPU:
  - Builds a transformer with 5 encoder and 5 decoder layers, 8 attention
    heads, d_model=512.
...
Usage:
    python validate_figure1_8layers.py
    python validate_figure1_8layers.py --duration 60 --interval 0.05
"""
...
# --------------------------------------------------------------------------- #
# Custom transformer with 5 encoder and 5 decoder layers:
#   5 encoder layers, 5 decoder layers, 8 attention heads,
...
def plot_transformer_trace(ax, samples, phases, t_start):
    ...
    ax.set_title("(a) Transformer Model (5 Enc / 5 Dec Layers)")
...
def main():
    ...
    parser.add_argument("--output", type=str, default="figure1_5layers_validation.png")
    ...
    print("[1/2] Running custom Transformer "
          f"(5 enc / 5 dec layers, 8 heads, d_model=512, seq_len={args.seq_len}, "
    ...
    fig.suptitle("Power Trace: 5-Layer Transformer vs CNN (Modified Figure 1)", fontsize=14, fontweight='bold')
    plt.tight_layout()
    ...
    # Summary report
    print("\n" + "="*70)
    print("SUMMARY REPORT: 5-Layer Transformer Inference Power Analysis")
    print("="*70)
    print(f"Transformer Architecture:")
    print(f"  - Encoder Layers: 5")
    print(f"  - Decoder Layers: 5")
